    ...
    .controller('themSuaVBNghiPhepCtrl', [
        ...
        function (
            ...
            var apiCreateUrl = createPdfTool == COMMON.CONG_CU_TAO_PDF.CRYSTAL_REPORT ?
                ...
                "api/QLNghiPhep/ThemVanBanCH";

            var apiUpdateUrl = createPdfTool == COMMON.CONG_CU_TAO_PDF.CRYSTAL_REPORT ?
                "api/QLNghiPhep/CapNhatVanBanCR" :
                "api/QLNghiPhep/CapNhatVanBanCH";
            ...
            function Init() {
                ...
                if (idselect.IDVanBan == null) {
                    ...
                    $ctrl.taoGiup = false;
                    GetDanhSachNhanVien($ctrl.Userdata.employeeId);
                }
                else {
                    $ctrl.TieuDeForm = COMMON.FORM.CAP_NHAT;
                    $ctrl.taoGiup = parseInt(idselect.MaNhanVien) == parseInt($ctrl.Userdata.employeeId) ? false : true;
                ...
                var resp = ApiClient
                    ...
                    .then(
                        function successCallback(response) {
                            ...
                            $ctrl.TypeFile.unshift({
                                ID: 19,
                                CODE: "3",
                                LOAIDM: "Loại file đính kèm",
                                VALUENAME: "|jpg|",
                                VALUENAMECODE: "TypeFile"
                            });
                        },
            ...
            function GetNgayNghiLe() {
                var resp = ApiClient
                    .get("api/QLNghiPhep/GetNgayNghiLe")
                    .then(
                        function successCallback(response) {
                            ...
                            $ctrl.NgayNghiLe.forEach((item) => {
                                ...
                                if (
                                    endDate_min.getDate() === parseInt(ngay[2]) &&
                                    endDate_min.getMonth() + 1 === parseInt(ngay[1]) &&
                                    endDate_min.getFullYear() === parseInt(ngay[0])) {
                                    // Trừ đi 2 ngày
            ...
            function GetQuyenUser() {
                var resp = ApiClient
                    ...
                    .then(
                        function successCallback(response) {
                            $ctrl.quyenuser.TaoVBKhongGioiHanBienDo = response.data
                                .findIndex(x => x.PermissionAction == 'CRP_NOLIMIT_NP') > -1;
                        },
                        function errorCallback(response) {
                        }
                    );
            ...
            function GetDanhSachNhanVien(manv) {
                var resp = ApiClient
                    .get(`api/QLNghiPhep/GetDanhSachNhanVienTrongPhong?maNhanVien=${manv}`)
                    .then(
                        function successCallback(response) {
            ...
            function GetThongTinNghiPhep(manv, isGetDanhSachDon) {
                var resp = ApiClient
                    .get(`api/QLNghiPhep/GetThongTinNghiPhepNhanVien?maNhanVien=${manv}&denNgay=${$ctrl.NgayNghiEnd.toDateString()}`)
                    .then(
                        function successCallback(response) {
                            ...
                            $ctrl.taoGiup = parseInt($ctrl.ThongTinNghiPhep.maNhanVien) == parseInt($ctrl.Userdata.employeeId) ? false : true;
                            if (isGetDanhSachDon) {
                                GetDanhSachDon();
                            }
                        },
                ...
                var resp = ApiClient
                    ...
                    .then(
                        function successCallback(response) {
                            ...
                            // bỏ mẫu của phó phòng/nv (nếu người đang đăng nhập là trưởng)
                            if (vaitroky.includes($ctrl.IDChucVu)) {
                                $ctrl.dsDon = $ctrl.dsDon.filter(x => x.ID != 3716 && x.ID != 3748);
                            }

                            // bỏ mẫu của trưởng phòng (nếu người đang đăng nhập là phó/nv)
                            if (!vaitroky.includes($ctrl.IDChucVu)) {
            ...
            function GetNguoiKyDuyet() {
                ...
                var resp = ApiClient
                    .postData("api/QLNghiPhep/GetNguoiKyDuyet", $.param({
                        valint1: result.Loai,
                        valint2: parseInt($ctrl.para.maNhanVien),
                        valstring1: $ctrl.ThongTinNghiPhep.idPhongBan,
                        valstring2: $ctrl.ThongTinNghiPhep.idChucVu
                    }))
                    .then(
                        function successCallback(response) {
            ...
            function GetTieuDeLoaiDon(type) {
                let loai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_NHAN_VIEN.TYPE;
                let messLoai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_NHAN_VIEN.MESSAGE;
                switch (parseInt(type)) {
                    case MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_NHAN_VIEN.CODE:
                        loai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_NHAN_VIEN.TYPE;
                        messLoai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_NHAN_VIEN.MESSAGE;
                        break;
                    case MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_TRUONG_PHONG.CODE:
                        loai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_TRUONG_PHONG.TYPE;
                        messLoai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_TRUONG_PHONG.MESSAGE;
                        break;
                    case MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_KHONG_LUONG.CODE:
                        loai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_KHONG_LUONG.TYPE;
                        messLoai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_KHONG_LUONG.MESSAGE;
                        break;
                    case MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_DI_NUOC_NGOAI.CODE:
                        loai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_DI_NUOC_NGOAI.TYPE;
                        messLoai = MODULES.DON_NGHI_PHEP.DON_NGHI_PHEP_DI_NUOC_NGOAI.MESSAGE;
                        break;
                    case MODULES.DON_NGHI_PHEP.DON_GIAI_TRINH_FACEID_NHAN_VIEN.CODE:
                        loai = MODULES.DON_NGHI_PHEP.DON_GIAI_TRINH_FACEID_NHAN_VIEN.TYPE;
                        messLoai = MODULES.DON_NGHI_PHEP.DON_GIAI_TRINH_FACEID_NHAN_VIEN.MESSAGE;
                        break;
                    case MODULES.DON_NGHI_PHEP.DON_GIAI_TRINH_FACEID_TRUONG_PHONG.CODE:
                        loai = MODULES.DON_NGHI_PHEP.DON_GIAI_TRINH_FACEID_TRUONG_PHONG.TYPE;
                        messLoai = MODULES.DON_NGHI_PHEP.DON_GIAI_TRINH_FACEID_TRUONG_PHONG.MESSAGE;
                        break;
                    default:
                        break;
                }
                ...
                let resp = ApiClient
                    ...
                    .then(
                        function successCallback(response) {
                            ...
                            setTimeout(function () {
                                blockUI.stop();
                                notificationService.success("Tạo mới/Cập nhật văn bản thành công");
            ...
            uploader.onSuccessItem = function (fileItem, response, status, headers) {
                if (response != 0) {
                    ...
                    notificationService.error("Có lỗi xảy ra khi tải File đính kèm, vui lòng kiểm tra lại định dạng hoặc dung lượng tập tin");
                }
            };
            ...
            function xoaFile(id) {
                blockUI.start();
                var resp = ApiClient
                    .postData("api/QLDieuXe/RemoveFile_Update", $.param({
                        valint1: id,
                        valint2: $ctrl.para.Loai
                    }))
                    .then(
                        function successCallback(response) {
                            //alert(response.data);
            ...
            function addFile(item) {
                blockUI.start();
                var resp = ApiClient
                    .postData("api/QLDieuXe/UploadFile_Update", $.param({
                        MoTa: item.MOTA,
                        LoaiFile: item.LOAIFILE,
                        VanBanID: $ctrl.para.id,
                        SizeFile: item.SIZEFILE,
                        Module: idselect.Loai
                    }))
                    .then(
                        function successCallback(response) {
                            blockUI.stop();
            ...
            $ctrl.DoiLoaiVB = function () {
                ...
                if (
                    $ctrl.Filter.FilterVanBan.LoaiVB == 28 ||
                    $ctrl.Filter.FilterVanBan.LoaiVB == 29) {
                ...
                // Loại mẫu đơn chọn là đơn nghỉ phép thường niên mẫu 1 và mẫu 2 và không lương và xin đi nước ngoài
                    ...
                    $ctrl.Filter.FilterVanBan.LoaiVB == 6 ||
                    $ctrl.Filter.FilterVanBan.LoaiVB == 7 ||
                    $ctrl.Filter.FilterVanBan.LoaiVB == 8 ||
                    $ctrl.Filter.FilterVanBan.LoaiVB == 9) {
            ...
            $ctrl.LuuNhap = function () {
                if ($ctrl.taoGiup == true && $ctrl.para.FileDinhKem.length == 0) {
                    notificationService.error("Đơn nghỉ phép/Đơn giải trình tạo giúp cần phải đính kèm file scan đã được ký bởi người xin nghỉ phép");
                    return;
                }
                if ($ctrl.para.DanhSachNguoiKy.length > maxNguoiDuyet || $ctrl.para.DanhSachNguoiKy.length == 0) {
                    thongnotificationServicebao.error("Đơn nghỉ phép/Đơn giải trình cần chọn tối đa " + maxNguoiDuyet + " và tối thiểu 1 người ký duyệt");
                    return;
                }
                ModalService.open({
                    ...
                    function successCallback() {
                        ...
                        var resp = ApiClient
                            .postData(apiCreateUrl, $.param($ctrl.para))
                            .then(
                                function successCallback(response) {
                                    //console.log(response.data);
                                    $ctrl.para.id = response.data.Id;

                                    if (createPdfTool == COMMON.CONG_CU_TAO_PDF.CRYSTAL_REPORT) {
            ...
            $ctrl.CapNhat = function () {
                if ($ctrl.taoGiup == true && $ctrl.para.FileDinhKem.length == 0) {
                    notificationService.error("Đơn nghỉ phép/Đơn giải trình tạo giúp cần phải đính kèm file scan đã được ký bởi người xin nghỉ phép");
                    return;
                }
                if ($ctrl.para.DanhSachNguoiKy.length > maxNguoiDuyet || $ctrl.para.DanhSachNguoiKy.length == 0) {
                    notificationService.error("Đơn nghỉ phép/Đơn giải trình cần chọn tối đa " + maxNguoiDuyet + " và tối thiểu 1 người ký duyệt");
                    return;
                }
                ModalService.open({
                    templateUrl: 'ConfirmModal.html',
                    ...
                    function successCallback() {
                        ...
                        //console.log($ctrl.para);
                        var resp = ApiClient
                            .postData(apiUpdateUrl, $.param($ctrl.para))
                            .then(
                                function successCallback(response) {
                                    if (createPdfTool == COMMON.CONG_CU_TAO_PDF.CRYSTAL_REPORT) {
                                        LoadReport($ctrl.para);
